...
export default function RecruiterSettingsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        showError(e?.message || 'Failed to load settings', 'Error')
      }
    }
    load()
  }, [])
  ...
  const onSave = async () => {
    ...
    try {
      ...
      if (res.success) showSuccess('Recruiter settings updated', 'Saved')
    } catch (e: any) {
      showError(e?.message || 'Failed to save settings', 'Error')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-3xl">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Recruiter Settings</CardTitle>
          <CardDescription>Manage your company profile & contact details</CardDescription>
        </CardHeader>
